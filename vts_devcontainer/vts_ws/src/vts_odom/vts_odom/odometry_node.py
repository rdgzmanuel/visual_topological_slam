...
class OdometryNode(Node):
    def __init__(self) -> None:
        ...
        self._trajectory: str = (
            self.get_parameter("trajectory").get_parameter_value().string_value
        )
    ...
    def _publish_odometry(self) -> None:
        ...
        odometry_path: str = os.path.join(
            trajectory_folder, odometry_folder, odometry_file
        )

        try:
            with open(odometry_path) as file:
        ...
        self.odom_data: list[list[float]] = [
    ...
    def _timer_callback(self) -> None:
        ...
        if self.odom_index == 0:
            self.prev_timestamp: float = float(
                f"{int(line_content[3])}.{int(line_content[4])}"
            )
        ...
        v, w = self.odometry._compute_poses(
            time_difference, x, y, theta, self.prev_x, self.prev_y, self.prev_theta
        )
